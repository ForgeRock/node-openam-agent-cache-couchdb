var Promise = require('bluebird'),
    util = require('util'),
    Connection = require('cradle').Connection,
...
CouchDBCache.prototype.get = function (key) {
    ...
    return Promise.promisify(this.db.get.bind(this.db))(key).then(function (res) {
        if (!res) {
...
CouchDBCache.prototype.put = function (key, value) {
    return Promise.promisify(this.db.save.bind(this.db))(key, {data: value, timestamp: new Date()});
};
...
CouchDBCache.prototype.remove = function (key) {
    ...
    return this.get(key).then(function (res) {
        return Promise.promisify(self.db.remove.bind(self.db))(key, res._rev);
    });
};
